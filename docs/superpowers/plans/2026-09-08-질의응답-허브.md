...
// domains/ask/worker/wrangler.jsonc
{
  ...
  "vars": {
    // secret 이 아니다. Task 0 스파이크가 통과시킨 슬러그를 넣고, 품질이 모자라면 이 값만 바꾼다
    "ASK_MODEL": "<Task 0 에서 확정한 슬러그>",
    "ASK_DAILY_QUOTA": "30",
...
- Consumes: `deps.db`
- Produces:
  - `TYPES` (6종 배열), `SLOT_KEYS` (6종 배열)
  - `normalize(raw, {어휘}) -> {유형, 슬롯, 확신도, 저확신, 저확신사유[]}`
...
test('주제가 어휘에 없으면 확신도와 무관하게 저확신이다', () => {
  // LLM 자기보고만 믿지 않는다
...
test('집단축이 어휘 밖이면 저확신이다', () => {
  const r = normalize({ ...ok, 슬롯: { ...ok.슬롯, 집단축: ['혈액형'] } }, { 어휘 });
  assert.equal(r.저확신, true);
});
...
export function normalize(raw, { 어휘 = {} } = {}) {
  ...
  const 슬롯 = { ...EMPTY, ...(r.슬롯 ?? {}) };
  슬롯.집단축 = Array.isArray(슬롯.집단축) ? 슬롯.집단축 : [];
  슬롯.출처 = Array.isArray(슬롯.출처) ? 슬롯.출처 : [];
  슬롯.기간 = { from: '', to: '', ...(슬롯.기간 ?? {}) };

  const 확신도 = typeof r.확신도 === 'number' ? r.확신도 : 0;
  if (확신도 < THRESHOLD) 사유.push(`확신도 ${확신도} < ${THRESHOLD}`);

  // 코드 측 신호 — LLM 자기보고만 믿지 않는다
  if (!슬롯.주제) 사유.push('주제가 비었다');
  else if (어휘.주제?.length && !어휘.주제.includes(슬롯.주제))
    사유.push(`주제가 카탈로그 어휘에 없다: ${슬롯.주제}`);
  for (const ax of 슬롯.집단축) {
    if (어휘.집단축?.length && !어휘.집단축.includes(ax))
      사유.push(`집단축이 어느 capability 에도 없다: ${ax}`);
  }

  const 저확신 = 사유.length > 0;
  return { 유형: 저확신 ? FALLBACK : 유형, 슬롯, 확신도, 저확신, 저확신사유: 사유 };
}
```
...
const SYS_CLASSIFY =
  '고용통계 질문을 스키마대로 분해만 한다. 답하지 않는다. 모르면 확신도를 낮게 준다.';
const SYS_COMPOSE =
