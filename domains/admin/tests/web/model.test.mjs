...
// **이 파일은 벽시계를 안 본다.** 도메인현황·경과시간은 `지금` 을 안 넘기면
// Date.now() 로 떨어진다. 그러면 고정 입력이 24시간을 넘긴 날부터
// '시간째 안 돌았다' 경고가 끼어들어, 시각과 무관한 단정까지 깨진다 —
// 2026-09-14 07:45 UTC 에 초록 크기 테스트가 실제로 그렇게 깨졌고, 배포는
// 이 테스트를 통과해야 나가므로 그날부터 사이트가 얼어붙었다(수집은 계속
// 커밋됐는데 화면만 옛 숫자를 26시간째 보여줬다). 한 줄 빠뜨린 것이
// 하루 뒤에 배포를 세우는 구조라서, 주입을 잊어도 안전하게 시계를 끊는다.
Date.now = () => 지금;
...
test('reports 는 초록 gzip 크기를 보이고 3MB 를 넘으면 경고한다', () => {
  ...
  // 경고가 0건인지가 아니라 **쪼갤 경고가 없는지**를 본다. 이 테스트의 주제는
  // 초록 크기뿐이라, 다른 축(경과시간·미판정)의 경고까지 세면 남의 일로 깨진다.
  assert.ok(!안넘음.경고.some((w) => w.includes('쪼갤 때다')));
});
