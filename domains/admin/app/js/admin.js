...
import { 불러오기, 토큰쓰기, 토큰지우기, 주인등록, 주인상태 } from './api.js';
...
async function 새로고침() {
  ...
  // 미배포·연결실패 상태에도 .tools 를 같이 그린다 — 안 그리면 토큰 지우기
  // 버튼도 사라져, 배포 도중 토큰을 잘못 넣었을 때 이 화면에 갇혀 빠져나갈
  // 길이 없어진다(localStorage 를 손으로 지워야 했다).
  else 화면().innerHTML = `<p class="empty">${안내[r.상태]}</p>${도구HTML(주인상태(localStorage))}`;
}
