...
// 수집 상태(API 검증/자동추출/확인완료)와 OCR 확인필요 라벨은 싣지 않는다.
// 홈·기관·비교에서 뺐으므로 여기만 남기면 왜 이 화면에만 있는지 설명이 안 된다.
function renderEventCard(event, today) {
  ...
  const subLine = `${esc(mmdd(event.published_at))} · ${esc(String(indicatorCount))}개 지표 갱신`;
