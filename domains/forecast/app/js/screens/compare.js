import { compareSet, summarize, fmtValue, fmtNumber, esc } from '../data.js';
...
// 흐린 막대가 무슨 뜻인지는 여기서만 설명된다. 이 줄을 지우려면 흐리게 그리는
// 것(compareSet 의 stale)도 같이 지워야 한다 — 설명 없는 흐림은 고장으로 보인다.
function renderWarning() {
  return `
    <div style="margin:8px 16px 0 16px;padding:10px 14px;background:var(--warn-bg);border-radius:10px;display:flex;gap:8px;align-items:flex-start;">
      ...
      <div style="font-size:12px;line-height:1.5;color:var(--warn-fg);">발표 시점이 상이하므로 단순 비교에 유의하세요. 3개월 이상 지난 전망은 흐리게 표시됩니다.</div>
    </div>`;
}

export function render(el, ctx) {
  const { records, indicatorMeta, state, today } = ctx;
