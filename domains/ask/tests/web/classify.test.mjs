...
test('자기보고 확신도는 거부권이 아니다 — 매핑이 되면 통과한다', () => {
  // 실측(gpt-4o-mini, 2026-09-09): 슬롯을 완벽히 내고도 확신도를 0.5 로 낸다.
  // 자기보고에 거부권을 주면 정상 질문이 전부 메타한계로 떨어진다.
  // 거부권은 객관 신호(매핑 실패)가 갖고, 확신도는 바닥 아래일 때만 신호다.
  const r = normalize({ ...ok, 확신도: 0.5 }, { 어휘 });
  assert.equal(r.유형, '수치조회');
  assert.equal(r.저확신, false);
});

test('자기보고 확신도는 판정에 전혀 쓰지 않는다 — 0 이어도 통과한다', () => {
  // 실측(배포된 워커, 같은 질문 4회): 슬롯은 네 번 다 완벽한데 확신도가
  // 0.5 · 0 · 1 · 0.5 로 나왔다. 정답을 내면서 0 을 내는 값에는 임계가 의미 없다.
  // 저확신은 객관 신호(어휘 매핑 실패)로만 정한다.
  for (const 확신도 of [0, 0.2, 0.5, 1]) {
    const r = normalize({ ...ok, 확신도 }, { 어휘 });
    assert.equal(r.유형, '수치조회', `확신도 ${확신도}`);
    assert.equal(r.저확신, false, `확신도 ${확신도}`);
    assert.equal(r.확신도, 확신도);          // 값 자체는 표시용으로 실린다
    assert.ok(!r.저확신사유.some((s) => s.includes('확신도')));
  }
});

test('매핑 실패는 확신도가 1 이어도 저확신이다', () => {
  const r = normalize({ ...ok, 확신도: 1, 슬롯: { ...ok.슬롯, 주제: '카페 매출' } }, { 어휘 });
  assert.equal(r.저확신, true);
  assert.equal(r.유형, '메타한계');
});

test('주제가 없어도 되는 유형이 있다', () => {
  // "경활과 행정통계는 왜 다른가" 에 주제가 무엇인가. 소스를 묻지 지표를 묻지 않는다.
  for (const 유형 of ['출처비교', '메타한계', '범위밖']) {
    const r = normalize({ 유형, 확신도: 0.9, 슬롯: { ...ok.슬롯, 주제: '' } }, { 어휘 });
    assert.equal(r.저확신, false, 유형);
    assert.equal(r.유형, 유형);
  }
});

test('주제가 필요한 유형에서 주제가 비면 저확신이다', () => {
  const r = normalize({ ...ok, 슬롯: { ...ok.슬롯, 주제: '' } }, { 어휘 });
  assert.equal(r.저확신, true);
  assert.ok(r.저확신사유.some((s) => s.includes('주제가 비었다')));
});
...
// ── 산업·직업 값 (실배포 화면 검증) ────────────────────────────────────────
// "제조업"이 집단축=산업으로만 접히고 category 가 {} 로 비어 **어느 산업인지가
// 사라졌다.** 연령·성 값은 표에 있는데 산업·직업 값만 없었다.
//
// 산업 코드는 D1 observation.category 실측(KSIC 대분류 한 글자 A~U)에 맞춘다.
// 직업 코드는 KSCO 대분류 한 자리다 — **관측이 아직 없는 축이지만 값은 살린다.**
// 값을 버리면 "직업축을 못 준다" 가 아니라 "무슨 말인지 모르겠다" 가 되어,
// 이 프로젝트가 지켜 온 `데이터미보유` vs `메타한계` 구분이 무너진다.

test('제조업이 산업 코드로 남는다', () => {
  const r = normalize({ 유형: '수치조회', 슬롯: { 주제: '취업자수', 집단축: ['제조업'] } },
    { 어휘: { 주제: ['취업자수'], 집단축: ['산업', '성', '연령'] } });
  assert.deepEqual(r.슬롯.집단축, ['산업']);
  assert.equal(r.슬롯.category.산업, 'C');
});

test('산업 대분류를 두루 안다', () => {
  const 어휘 = { 주제: ['취업자수'], 집단축: ['산업', '성', '연령'] };
  const 코드 = (v) => normalize({ 유형: '수치조회', 슬롯: { 주제: '취업자수', 집단축: [v] } },
    { 어휘 }).슬롯.category.산업;
  assert.equal(코드('건설업'), 'F');
  assert.equal(코드('도소매업'), 'G');
  assert.equal(코드('숙박음식업'), 'I');
  assert.equal(코드('보건복지업'), 'Q');
  assert.equal(코드('정보통신업'), 'J');
});

test('직업 값도 살아남는다 — 관측이 없다는 것과 못 알아듣는 것은 다르다', () => {
  const r = normalize({ 유형: '원인탐색', 슬롯: { 주제: '취업자수', 집단축: ['사무직'] } },
    { 어휘: { 주제: ['취업자수'], 집단축: ['산업', '성', '연령'] } });
  assert.deepEqual(r.슬롯.집단축, ['직업']);
  assert.equal(r.슬롯.category.직업, '3');
});

test('산업과 연령을 같이 물으면 둘 다 남는다', () => {
  const r = normalize({ 유형: '수치조회',
    슬롯: { 주제: '취업자수', 집단축: ['제조업', '청년'] } },
    { 어휘: { 주제: ['취업자수'], 집단축: ['산업', '성', '연령'] } });
  assert.equal(r.슬롯.category.산업, 'C');
  assert.equal(r.슬롯.category.연령, '15-29');
});
