...
// ── 상대 기간: 오늘을 넘겨야 풀린다 ──────────────────────────────────────
//
// 기준 시각은 **KST** 다. UTC 로 재면 한국 시각 오전 9시 전에는 날짜가 하루 어긋나고,
// 12월 31일·1월 1일에는 연도가 어긋난다.
const 어느날 = new Date('2026-09-13T04:00:00Z');   // KST 2026-09-13 13:00

test('작년을 그 해 전체로 푼다', () => {
  assert.deepEqual(slotsFromText('작년 취업자', { now: 어느날 }).기간,
    { from: '2025-01', to: '2025-12' });
});

test('작년 8월은 작년의 그 달이다', () => {
  assert.deepEqual(slotsFromText('작년 8월 취업자', { now: 어느날 }).기간,
    { from: '2025-08', to: '2025-08' });
});

test('지난달은 한 달 물러난다 — 1월에서 0월이 나오지 않는다', () => {
  assert.deepEqual(slotsFromText('지난달 취업자', { now: new Date('2026-01-20T04:00:00Z') }).기간,
    { from: '2025-12', to: '2025-12' });
});

test('연도 없는 달은 올해로 읽는다', () => {
  assert.deepEqual(slotsFromText('7월 취업자', { now: 어느날 }).기간,
    { from: '2026-07', to: '2026-07' });
});

test('아직 오지 않은 달은 작년이다', () => {
  // 9월에 "12월 취업자" 를 묻는 사람은 없는 값(올해 12월)을 묻는 게 아니다.
  assert.deepEqual(slotsFromText('12월 취업자', { now: 어느날 }).기간,
    { from: '2025-12', to: '2025-12' });
});

test('최근 6개월은 창으로 푼다', () => {
  assert.deepEqual(slotsFromText('최근 6개월 취업자', { now: 어느날 }).기간,
    { from: '2026-04', to: '2026-09' });
});

test("'최근' 만으로는 기간을 잡지 않는다", () => {
  // 창을 억지로 잡으면 그 창 밖의 글이 통째로 사라진다 — 2019년 보고서가 답인
  // 질문에서 빈손이 된다. 최신성은 순위로 푼다(search.mjs).
  assert.deepEqual(slotsFromText('최근 고용상황은?', { now: 어느날 }).기간,
    { from: '', to: '' });
});

test('KST 로 잰다 — UTC 로 재면 연도가 어긋나는 순간이 있다', () => {
  // UTC 2025-12-31 16:00 은 KST 2026-01-01 01:00 이다. '올해' 는 2026 이어야 한다.
  assert.deepEqual(slotsFromText('올해 취업자', { now: new Date('2025-12-31T16:00:00Z') }).기간,
    { from: '2026-01', to: '2026-12' });
});

test('넓은 말도 주제로 받는다 — 안 받으면 숫자 없는 답이 나간다', () => {
  assert.equal(slotsFromText('일자리 사정 어때').주제, '취업자수');
  assert.equal(slotsFromText('노동시장 어떤가').주제, '취업자수');
  assert.equal(도메인적용('일자리 사정 어때').find((x) => x.도메인 === 'employment').적용, true);
});
...
test('행통 모니터링도 글로 찾는 도메인이다', () => {
  // 기사 제목과 회차 분석이 색인에 담겼다(2026-09-10). 목록에서 빠지면
  // 사용자는 그 도메인이 있는 줄도 모른다.
  const p = 도메인적용('청년 고용률').find((x) => x.도메인 === 'press');
  assert.ok(p, '행통 모니터링이 목록에 없다');
  assert.equal(p.적용, true);
});
