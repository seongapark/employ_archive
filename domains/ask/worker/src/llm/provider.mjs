...
- **자리수를 그대로 옮긴다. 반올림·어림하지 않는다.** 29151 을 "약 29,150" 이나 "2,915만"
  으로 쓰면 재료 밖의 숫자가 되어 문장이 통째로 버려진다(실측). 29,151 처럼 쉼표는 괜찮다.
...
- **개수를 말할 때는 재료의 '건수' 값을 그대로 쓴다.** 세어서 새로 만들지 않는다.
- **3~4문장, 300자 안.** 제목을 나열하지 않는다. 표·목록·머리글 없이 줄글로 쓴다.
- **숫자는 세 개까지.** 최신값, 1년 전과의 차이, 흐름이 뒤집힌 한 시점이면 충분하다.
  열두 달을 다 읊으면 읽는 사람이 요점을 잃는다(실측: 624자에 숫자 열넷).
