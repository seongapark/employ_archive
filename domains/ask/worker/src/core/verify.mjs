...
//   ③ 천 단위 구분자가 라벨에 먹힌다 — 뒤보기에 `,` 를 넣어 `1,234대` 의 `234대` 가
//      가려지지 않게 한다. 가려지면 남은 `1,` 에서 1 이 뽑혀 원래 값 1234 가 사라진다.
const COHORT = /(?<![\d.,])(?:\d+\s*[~\-–]\s*\d+세|\d+세\s*(?:이상|미만|이하|초과)?|\d+대)/g;
const TERM = /(?<![\d.,])\d+(?:분기|반기|월|년)/g;

// 천 단위 구분자를 한 숫자로 읽는다. LLM 은 29151 을 `29,151` 로 쓰는데, 이걸 모르면
// 29 와 151 이 근거에 없다며 **정상 답변이 통째로 버려진다**(실측: 2026-09-09).
// 쉼표+세 자리가 **붙어 있을 때만** 묶는다 — 나열은 `20, 151` 처럼 띄어 쓰므로 안 걸린다.
...
export function extractNumbers(text) {
  ...
  return (masked.match(NUM) ?? [])
    .map((s) => Number(s.replace('−', '-').replace(/,/g, '')))
