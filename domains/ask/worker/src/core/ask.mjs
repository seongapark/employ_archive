...
async function 현상찾기(deps, 슬롯) {
  ...
  // **축 값이 어긋나면 고르지 않는다.** 주제 단어 하나로 맞히면 현상이 늘수록 오답이
  // 는다 — 등록 현상이 하나뿐이던 시절 정의에 "취업자수" 가 들어 있어 취업자를 묻는
  // 모든 원인탐색이 여기로 걸렸고, "제조업이 왜 줄었냐" 에 "청년 사무직이 줄었다" 를
  // 답했다(2026-09-09 화면 실측).
  //
  // 규칙은 하나다 — **질문이 지목한 축은 전부 현상이 선언하고 값도 같아야 한다.**
  //   · 질문이 축 값을 하나도 안 주면(그냥 "왜 줄었어") 아무것도 안 걸러 주제 매칭만
  //     남는다. 그게 이 도구가 당연히 답해야 하는 질문이라서다.
  //   · 질문이 현상이 말한 적 없는 축을 지목하면 안 고른다. 산업=C 든 성=F 든 같다 —
  //     둘 다 그 현상이 가리지 않는 이야기고, 내밀면 **묻지 않은 것을 답한 셈**이다.
  //   · 질문이 선언 축의 일부만 지목하면 고른다("청년 취업자 어때" → 연령만).
  // 방향이 중요하다: 선언을 순회하면 산업=C 가 안 걸린다(현상이 산업을 안 적었으므로).
  // **슬롯을 순회**해야 걸린다.
  const 슬롯축 = 슬롯.category ?? {};
  const 축이맞나 = (p) => {
    let 선언;
    try { 선언 = JSON.parse(p.집단축 ?? 'null'); } catch { 선언 = null; }
    if (!선언 || typeof 선언 !== 'object') return Object.keys(슬롯축).length === 0;
    return Object.entries(슬롯축).every(([축, 값]) =>
      축 in 선언 && String(선언[축]) === String(값));
  };

  // 주제는 매핑된 값과 원문 둘 다로 맞춰 본다 — 카탈로그 문구가 어느 쪽을 담을지 모른다.
  const 열쇠 = [주제, 슬롯.주제, ...(슬롯.집단축 ?? [])].filter(Boolean);
  const 맞는것 = 후보.filter(축이맞나).find((p) => {
