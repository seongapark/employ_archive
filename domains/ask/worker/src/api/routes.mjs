...
import { 요약재료 } from '../lookup/summary.mjs';
...
// ── 출처 탐색 + 요약 ───────────────────────────────────────────────────────
//
// 질문에 맞는 출처를 도메인별로 모아 카드로 내고, **그 카드를 재료로 문장 하나를
// 덧붙인다**(2026-09-13. 그전까지는 카드만 냈다). 위의 `handleAsk`(옛 유형 분기 +
// 2패스 서술)는 여전히 호출하지 않는다 — 지우지 않았을 뿐이다.
//
// **찾기는 LLM 과 무관하다.** 출처 찾기는 D1 읽기뿐이라(한 질문에 최대 여덟 번)
// 할당량과 상관없이 언제나 돌고, 할당량이 막는 것은 문장 하나다. 그래서 한도초과·
// 확인불가·요약 실패·검증 실패 **네 경우 모두 카드는 그대로 나간다.**
...
export async function handleLookup(deps, { 질문, ip, today, now }) {
  ...
  // **카드는 언제나 나간다. 문장만 덧붙는다.** 한도초과·확인불가·요약 실패·검증
  // 실패가 전부 같은 자리에서 갈리며, 어느 경우든 출처 카드는 그대로다.
  if (!q.허용) return { ...r, 배지 };

  const { 재료, 허용텍스트, 허용숫자 } = 요약재료(r);
  let 문장 = null;
  try {
    문장 = await deps.llm?.요약?.(재료) ?? null;
  } catch (e) {
    // 소리 없이 삼키지 않는다 — 화면이 "출처는 있는데 문장이 없다" 를 낼 때 그게
    // 우리 LLM 장애인지 검증 탈락인지 갈라야 한다(옛 경로에서 배포를 한 번 더 썼다).
    console.error('요약 실패', e?.name, e?.message);
  }
  if (!문장) return { ...r, 배지: [...배지, '요약실패'] };

  // 숫자는 LLM 이 만들지 않는다. 재료 밖의 숫자가 하나라도 있으면 **문장만** 버린다.
  // 허용 집합은 요약재료가 실제로 넘긴 문자열·숫자 그대로다 — 넘긴 것과 허용하는
  // 것이 갈라지면 정상 문장이 버려지거나 환각이 통과한다.
  const v = verify({ 답변: 문장 }, [], 허용텍스트, 허용숫자);
  if (!v.ok) {
    return { ...r, 배지: [...배지, '검증실패'], 검증위반: v.위반, 검증진단: v.진단 };
  }
  return { ...r, 답변: 문장, 배지 };
}
