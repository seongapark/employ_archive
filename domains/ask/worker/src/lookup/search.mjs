...
// LIKE 로 훑어 합친다. 색인이 4,282행·10MB 남짓이라 전수 훑기가 아직 감당된다 —
// 데이터가 열 배가 되면 다시 잰다.
//
// 이 함수는 **조회만 한다.** 순위·중복 제거는 `추리기` 가, 확장어로 보충할지는
// `글조회` 가 정한다 — 셋을 한 함수에 두면 확장 조회를 끼울 자리가 없다.
async function 한판(deps, 도메인, 용어들, { 기간, limit, 최소겹침 }) {
  ...
  if (기간?.from || 기간?.to) {
    const 조건 = [];
    if (기간.from) { 조건.push('날짜 >= ?'); 창값.push(날짜아래(기간.from)); }
    if (기간.to) { 조건.push('날짜 <= ?'); 창값.push(날짜위(기간.to)); }
    창절 = ` AND (날짜 = '' OR (${조건.join(' AND ')}))`;
  }
  ...
  if (짧은것.length) {
    ...
    rows.push(...await deps.db.all(
      ...
      [도메인, ...값, ...값, 최소겹침, ...창값, ...값, limit]));
  }

  return rows;
}

// **문서 단위로 추린다.** 초록 한 건이 여러 청크로 쪼개져 있어, 조각 단위로 추리면
// 다섯 자리가 같은 보고서로 다 차서 다른 보고서를 볼 기회가 사라진다(실측). 두 길로
// 같은 글이 걸리는 것도 여기서 함께 걸러진다 — 점수가 높은 쪽이 앞에 있으므로 먼저
// 온 것을 남긴다.
function 추리기(정렬된행, 도메인, limit) {
  const 본것 = new Set();
  const out = [];
  for (const r of 정렬된행) {
...
// 원어로 찾고, **자리가 남으면 비슷한 말로 보충한다.**
//
// 확장은 대체가 아니라 보충이다 — 확장어로 걸린 행은 점수 0 이라 원어로 걸린 행 뒤에
// 선다. 그래서 원어만으로 자리가 차는 질문의 순위는 한 자리도 안 바뀌고, D1 왕복도
// 안 늘어난다(자리가 찼으면 두 번째 조회를 아예 안 던진다).
async function 글조회(deps, 도메인, 용어들, limit, 기간 = { from: '', to: '' }, 확장어 = []) {
  // 낱말이 셋 이상이면 **둘 이상 걸려야** 낸다. '고용' 처럼 거의 모든 글에 있는 말
  // 하나만 걸린 글을 "정년 연장" 질문의 답으로 내밀면 묻지 않은 것을 답한 셈이다(실측).
  const 최소겹침 = (용어들 ?? []).filter((x) => x.length < MIN).length >= 3 ? 2 : 1;
  const rows = await 한판(deps, 도메인, 용어들, { 기간, limit: limit * 여유, 최소겹침 });
  ...
  let out = 추리기(점수붙임, 도메인, limit);
  if (out.length >= limit || !확장어.length) return out;

  // 보충 조회는 **하나만 걸려도** 받는다(최소겹침 1). 어차피 점수 0 으로 뒤에 서므로
  // 넓게 받아도 상위를 밀어내지 않고, 좁게 받으면 빈 카드가 그대로 남는다.
  const 더 = await 한판(deps, 도메인, 확장어, { 기간, limit: limit * 여유, 최소겹침: 1 });
  const 보충 = 더.map((r) => ({ ...r, 점수: 0, 비슷한말: true }))
    .sort((a, b) => String(b.날짜 ?? '').localeCompare(String(a.날짜 ?? '')));
  return 추리기([...점수붙임, ...보충], 도메인, limit);
}
...
// 도메인마다 **결과가 있어도 밝혀야 하는 사실**이 있다. 전에는 사유를 빈손일 때만
// 달았는데, 기사는 걸렸을 때가 더 위험하다 — 제목만 색인돼 있어서 우리는 그 기사가
// 무슨 말을 했는지 모른다. 요약이 내용을 아는 척하지 않도록, 화면과 요약 재료 양쪽에
// 같은 문장으로 실어 둔다.
const 도메인안내 = {
  press: '기사는 제목만 색인돼 있다 — 무슨 내용인지는 원문을 훑어야 안다',
};

function 안내(도메인, 건수, 비슷한말수) {
  const 말 = [];
  if (!건수) 말.push('이 질문으로 걸리는 글이 없다');
  else if (도메인안내[도메인]) 말.push(도메인안내[도메인]);
  if (비슷한말수) 말.push(`${비슷한말수}건은 비슷한 말로 찾았다`);
  return 말.length ? { 사유: 말.join(' · ') } : {};
}
...
export async function lookup(deps, 질문, { limit = LIMIT, now } = {}) {
  ...
  // 비슷한 말. 원어로 자리가 안 차는 도메인만 이걸로 보충한다(`글조회` 참조).
  const 확장어 = 확장(용어들);

  for (const 도메인 of 글도메인) {
    if (!켜진.has(도메인)) continue;
    const rows = await 글조회(deps, 도메인, 용어들, limit, 슬롯.기간, 확장어);
    const 비슷한말수 = rows.filter((r) => r.비슷한말).length;
    묶음.push({
      도메인,
      결과: rows.map((r) => ({
        제목: r.제목,
        // 스니펫은 확장어까지 넘겨 찾는다 — 비슷한 말로 걸린 글에서 원어를 찾으면
        // 히트가 없어 글 앞부분이 잘려 나온다(왜 이 글이 올라왔는지 안 보인다).
        스니펫: 스니펫(r.본문, [...용어들, ...확장어]),
        링크: r.링크 ?? '', 근거유형: r.종류,
        날짜: r.날짜 ?? '', 점수: r.점수 ?? 0,
        ...(r.비슷한말 ? { 비슷한말: true } : {}),
      })),
      ...안내(도메인, rows.length, 비슷한말수),
    });
  }
