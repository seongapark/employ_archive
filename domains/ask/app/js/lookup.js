...
// 질문 하나의 주소. 화면과 허브가 같은 규칙을 써야 뒤로가기·새로고침이 같은 질문을 낸다.
// 순수 함수로 둔 이유는 제출 처리에서 **해시가 실제로 바뀌는지** 비교해야 하기 때문이다
// (안 비교하고 무조건 `go` 를 부르면 요청이 두 번 나간다 — ask.js 주석).
export const 질문해시 = (q) => `#/q=${encodeURIComponent(String(q ?? '').trim())}`;
