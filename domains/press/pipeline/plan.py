...
# 후속 구간은 D+1~D+21 이다(정기 D~D+1 과 합쳐 **배포일부터 3주**).
# **그 구간이 열려 있는 동안 매일 긁는다** — D+16 에 딱 한 번만 긁던 것을
# 2026-09-11 에 바꿨다. 한 번만 긁으면 후속 화면이 3주 내내 비어 있다가 어느 날
# 갑자기 차고, 그 사이에 프레임이 번져도 아무도 모른다. 이 도메인에서 가장
# 나쁜 것이 늦게 아는 것이다.
#
# D+1 은 빼 둔다 — 그날은 정기 수집이 도는 날이고, 후속 구간이 하루치뿐이라
# 긁을 것이 사실상 없다. D+22 는 구간이 닫힌 다음날이라 마지막으로 한 번 더 돈다.
FOLLOW_FROM = 2
FOLLOW_LAST = 22
...
def decide(today, releases):
    ...
    - 배포 +2일 ~ +22일 → 후속. 구간(D+1~D+21)이 열려 있는 동안 **매일**
      긁고, 닫힌 다음날(D+22)에 한 번 더 긁어 마감한다.
    - 그 밖의 날 → 아무것도 안 한다.
